...
id: T005
status: blocked
implementation_state: review_pending
priority: P0
task_type: component
component: C02
optional: false
...
`ADR-004` supplies what the source does not: two named, separately registered profiles with pinned parameters, and a default. That is an operational convention, not an official reading of section 4.3, and nothing in this task may describe it as one. It is sufficient to implement, select, declare, and lock a model, so this task does not wait on an official OPEN-009 answer and its `{#model_lock}` criterion is not gated.

### Current state (verified 2026-08-16)

Both profiles, the selection seam, and the lock are implemented on the feature branch `feature/issue-8-t005-scent-model-lock` (tracked by Issue #8). That branch is **not** merged into the integration branch and is not part of the verified baseline. `implementation_state` is `review_pending`: the work exists, has not been reviewed against the criteria below, and has not been integrated.

Two conditions must be satisfied before integration, in addition to the acceptance criteria:

1. **Third-party material review.** Any test fixture or profile implementation carried into this repository from an external source must be reviewed for licence obligations, and every legally required copyright, licence, and NOTICE text must ship with it. If a required notice is absent, either add it or replace the material with vectors generated by this repository from its own recorded profile parameters. Do not integrate the branch with the question unresolved.
2. **Deterministic re-verification.** The vectors below must pass on the integration branch after merge, not only on the feature branch.
...
The verification surface for this task is:

| Surface | How it is proven |
|---|---|
| Golden scent vectors | Committed vectors for both profiles reproduce exactly, with no tolerance for the rounded profile |
| Repeated emission | Repeated emission at one cell asserts each profile's own behavior — no upper clamp under `subtractive_chebyshev_v1`, clamping at `0.9` under `multiplicative_book_v1` |
| Corners and edges | Emission at every board corner and edge clips to the board without wrapping, reflecting, or redistributing intensity |
| Deterministic behavior | The same `(profile, config, action sequence)` produces byte-identical fields across repeated runs and fresh processes |
| Selected profile | With no explicit configuration the active profile is `subtractive_chebyshev_v1`; selection is configuration-driven and no consumer hardcodes a model |
| Model/config lock hash | The pinned parameter document hashes to a stable value; the same document always produces the same hash |
| Police/Thief agreement | Both role repositories run the same vectors and produce identical hashes and fields |
| Repository quality gates | Ruff, pytest with the coverage threshold, and the repository gate suite |
